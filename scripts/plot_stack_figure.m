...
%save warm_stack.mat WDstack NGstack CH4stack
...
%save cool_stack.mat WDstackc NGstackc CH4stackc
...
if showfigures
    ...
    %set(gcf,'outerposition',get(0,'screensize'));
    plot(t, NGstack/(max(NGstack)-min(NGstack))+1.1,'linewidth',2,'color',[0 0.2 0.9])

    hold on
    plot(t, CH4stack/(max(CH4stack)-min(CH4stack))+1.1,'linewidth',2,'color',[0 0.8 0])
    xlim([-600 800])
    plot(t, (WDstack-mean(WDstack))/(max(WDstack)-min(WDstack)),'linewidth',2,'color',[1 0.6 0])
    plot(t, (fit_curve-mean(WDstack))/(max(WDstack)-min(WDstack)),'linewidth',3,'color',[0.9 0.4 0])
    xlabel('Time (years)', 'FontWeight', 'Bold')
    ylabel('Normalized signal', 'FontWeight', 'Bold')
    ylim([-0.4 1.7])
    yrange = ylim;
    %h=rectangle('Position', [0, yrange(1), t_break, yrange(2)-yrange(1)], ...
    %    'facecolor', 'yellow', 'Edge','none');
    patch([0, 0, t_break, t_break],[yrange(1),yrange(2), yrange(2), yrange(1)],...
        [132,112,255]/255, 'FaceAlpha',0.2, 'EdgeColor', 'None');
    
    h = legend('NGRIP \delta^{18}O','WDC CH_4', 'WDC \delta^{18}O', 'Fit Curve');
    set(h, 'FontWeight', 'Bold', 'FontName', 'Helvetica', 'Location', 'northwest', 'Box','off')
    %set(gcf, 'PaperPositionMode', 'auto')   % Use screen size

    set(gcf, 'PaperPositionMode', 'auto')   % Use screen size
    print -djpeg -r200 ../figs/warm_stack.jpeg;


    figure;
    set(gcf,'outerposition',get(0,'screensize'));
    %set(gcf,'outerposition',get(0,'screensize'));
    plot(t, NGstackc/(max(NGstackc)-min(NGstackc))+0.8,'linewidth',2,'color',[0 0.2 0.9])
    hold on

    plot(t, CH4stackc/(max(CH4stackc)-min(CH4stackc))+0.8,'linewidth',2,'color',[0 0.8 0])
    xlim([-600 800])
    plot(t, (WDstackc-mean(WDstackc))/(max(WDstackc)-min(WDstackc)),'linewidth',2,'color',[1 0.6 0])
    plot(t, (fit_curvec-mean(WDstackc))/(max(WDstackc)-min(WDstackc)),'linewidth',2,'color',[0.9 0.4 0])
    xlabel('Time (years)', 'FontWeight','Bold')
    ylabel('Normalized signal','FontWeight','Bold')
    ylim([-0.7 1.4])
    yrange = ylim;
    patch( [0, 0, t_breakc, t_breakc],[yrange(1),yrange(2), yrange(2), yrange(1)],...
       [132,112,255]/255, 'FaceAlpha',0.2, 'EdgeColor', 'None');

    h = legend('NGRIP \delta^{18}O','WDC CH_4', 'WDC \delta^{18}O','Fit Curve');
    set(h, 'FontWeight', 'Bold', 'FontName', 'Helvetica', 'Location', 'best', 'Box', 'off')
    %text(t_break, 0.2, num2str(t_break))
    set(gcf, 'PaperPositionMode', 'auto')   % Use screen size
    print -djpeg -r200 ../figs/cool_stack.jpeg;
end
